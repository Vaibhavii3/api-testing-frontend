...
const Skills = () => {
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        const response = await axios.get("https://api-test-psi-sable.vercel.app/skill");
  ...
  const handleSave = async () => {
    try {
      if (currentSkill) {
        // Update skill
        await axios.put(`https://api-test-psi-sable.vercel.app/skill/${currentSkill._id}`, formData);
        setSkills((prevSkills) =>
          prevSkills.map((skill) => (skill._id === currentSkill._id ? { ...skill, ...formData } : skill))
        );
      } else {
        // Add skill
        const response = await axios.post("https://api-test-psi-sable.vercel.app/skill", formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this skill?")) {
      try {
        await axios.delete(`https://api-test-psi-sable.vercel.app/skill/${id}`);
